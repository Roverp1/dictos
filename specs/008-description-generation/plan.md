...
The AI adapter will start with Vercel AI SDK's OpenAI-compatible provider and non-streaming `json_object` output. The expected shape is included in the request and validated locally before the adapter returns. The adapter permits at most two AI SDK retries for retryable provider failures; core and the Command Client do not retry generation. Native provider `json_schema` modes, provider-specific SDKs, and streaming remain replaceable adapter concerns and are not part of v1. The core service performs a second semantic validation for requested Description Type coverage, non-empty content, valid duplicate candidate IDs, and target ownership. Expected failures cross every boundary as tagged Error values.
...
`@dictos/ai-sdk` implements this port with pinned, mutually compatible `ai` and `@ai-sdk/openai-compatible` versions recorded in `bun.lock`. It creates the provider with `createOpenAICompatible({ supportsStructuredOutputs: false })` and calls `generateText()` once with `Output.object({ schema })` and `maxRetries: 2`. The schema is a runtime JSON/Standard Schema definition for the relevant proposal shape. In this mode the provider request uses `json_object`; AI SDK parses and validates the result locally. The prompt explicitly requires JSON and includes an example of the exact proposal shape for providers such as DeepSeek. The package exposes a sanitized AI SDK warning bridge that the Command Client composition root routes to structured logs instead of command output. Access to the output getter is also wrapped at the adapter boundary because it may fail after the request resolves.

The adapter must accept an injected `fetch` implementation for boundary tests. One `generateText()` invocation may make the initial HTTP request plus at most two retries when AI SDK marks a failure retryable. The adapter does not stream, persist, or decide whether a duplicate is accepted.
...
The new adapter package depends on `@dictos/core`, `@dictos/errore`, `@dictos/logger`, `ai`, and `@ai-sdk/openai-compatible`. Core does not import any AI SDK type. The adapter accepts injected `fetch`, making provider HTTP the only mocked boundary in its tests.
...
- Runner-neutral core repository contracts use real adapter harnesses to verify default Sense detachment, explicit cascade deletion and rollback, existing-Sense target identity, strict stale proposal rejection, and full rollback after a controlled failure following the first write. Core service tests cover same-Entry assignment behavior.
- The runner-neutral `@dictos/db-core/testing` contract verifies tables, indexes, foreign keys, checks, and lifecycle triggers through the fresh Bun migration path. The WASM migration path must be verified separately through cross-platform browser E2E against fresh OPFS storage; `@dictos/wasm-turso-sync` has no package-local browser runner.
...
- AI adapter tests inject a deterministic HTTP boundary and verify `json_object` parsing, local schema validation, provider errors, invalid output, Model discovery, sanitized warning logging, and the initial request plus at most two retries for retryable failures. No paid provider call runs in the default test suite.
